...
rows.forEach(function (row) {
    const firstSixCells = row.querySelectorAll('td');

    for (let i = 0; i < Math.min(firstSixCells.length, 6); i++) {
        firstSixCells[i].addEventListener('click', function () {
            const link = this.querySelector('a') ? this.querySelector('a').href : null;
            if (link) {
...
document.querySelector('#previousPage').addEventListener('click', function () {
    ...
    if (Number(currentPageButton.innerText) > 1) {
        ...
        updatePage(targetPage, getSelectedRadio());
...
function pageButtonAddEvent() {
    document.querySelectorAll('.page-num').forEach(button => {
        button.addEventListener('click', () => {
            ...
            updatePage(targetPageID, getSelectedRadio());
...
document.querySelector('#lastPage').addEventListener('click', function () {
    ...
    updatePage(totalPages, getSelectedRadio());
    changePagebuttons(lastPageButton);
});

//取得該頁的訂單資料&設定頁數按鈕
async function updatePage(pageID, statusID) {
    try {
        const response = await axios.get(`statusSort/${statusID}?p=${pageID}`);

        if (Number(statusID) <= 1) {
            statusTitle.innerText = statusTimeList[0];
        } else {
            statusTitle.innerText = statusTimeList[statusID - 1];
        }
        // 更新總元素和總頁數
        totalElements = response.data.page.totalElements;
        totalPages = response.data.page.totalPages;

        // 檢查 _embedded 和 orderList 是否存在
...
async function statusChangeAction(option) {
    ...
    axios.post('update', {
        ...
        .then(response => {
            ...
            const currentPage = Number(document.querySelector('.page-num.active').innerText);
            const currentStatus = getSelectedRadio();
            updatePage(currentPage, currentStatus);
        })
        .catch(error => {
            console.error(error);
        })
